...
export default function renderCard(filmsList) {
  const customList = filmsList.map(elem => {
    ...
    if (elem.genre_ids == '' || elem.genre_ids === 'no genres') {
      ...
      console.log('no genres if', elem.genre_ids);
      console.log('no genres if', elem.genre_ids.length);
    } else {
      // new rules for single card render
      if (elem.genre_ids) {
        console.log(elem.genre_ids);
        console.log('no genres else ', elem.genre_ids.length, elem.genre_ids);
        ...
        if (elem.genres.length === 0) {
          elem.genre_ids = 'no genres';
          console.log('если пустой');
        } else {
          console.log('elem.genres else if: ', elem.genres);
          elem.genre_ids = elem.genres.map(el => el.name).join(' ');
          console.log('elem.genres else if: ', elem.genre_ids);
          console.log('если с Данными');
        }
      }
    }
